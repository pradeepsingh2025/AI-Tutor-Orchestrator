...
class EducationalToolClient:
    ...
    async def call_note_maker(self, request: NoteMakerRequest) -> NoteMakerResponse:
        ...
        try:
            response_data = await self._make_request(
                url=self.note_maker_url, payload=request.model_dump()
            )
    ...
    async def call_flashcard_generator(
        ...
        try:
            response_data = await self._make_request(
                url=self.flashcard_url, payload=request.model_dump()
            )
    ...
    async def call_concept_explainer(
        self, request: ConceptExplainerRequest
    ) -> ConceptExplainerResponse:
        """
        ...
        try:
            response_data = await self._make_request(
                url=self.concept_explainer_url, payload=request.model_dump()
            )
    ...
    async def _make_request(
        self, url: str, payload: Dict[str, Any], retry_count: int = 0
    ) -> Dict[str, Any]:
        """
...
class MockToolClient(EducationalToolClient):
    ...
    async def call_flashcard_generator(
        ...
        logger.info(
            f"[MOCK] Generating {request.count} flashcards for: {request.topic}"
        )
            ...
            flashcards.append(
                {
                    ...
                    "example": (
                        f"Example: {request.topic} in practice"
