...
Manages the workflow from student message to tool execution
"""
from typing import TypedDict, Annotated, Literal
from langgraph.graph import StateGraph, END
from models import (
    UserInfo, ChatMessage, ExtractedParameters, ToolSelection,
    NoteMakerRequest, FlashcardRequest, ConceptExplainerRequest,
...
class AITutorOrchestrator:
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        workflow.add_node("execute_flashcard_generator", self.execute_flashcard_generator)
    ...
    def analyze_message(self, state: OrchestratorState) -> OrchestratorState:
        """
        Step 1: Analyze student message and extract parameters
        """
        ...
        try:
            ...
            logger.info(f"Extracted tool: {extracted.tool_needed}, confidence: {extracted.confidence}")
    ...
    def validate_parameters(self, state: OrchestratorState) -> OrchestratorState:
        """
        Step 2: Validate extracted parameters and check completeness
        """
        ...
        if extracted.missing_parameters:
            ...
            state["clarification_questions"] = self._generate_clarification_questions(extracted)
    ...
    async def execute_note_maker(self, state: OrchestratorState) -> OrchestratorState:
        """
        Step 3a: Execute Note Maker tool
        """
    ...
    async def execute_flashcard_generator(self, state: OrchestratorState) -> OrchestratorState:
        """
        Step 3b: Execute Flashcard Generator tool
        """
    ...
    async def execute_concept_explainer(self, state: OrchestratorState) -> OrchestratorState:
        """
        Step 3c: Execute Concept Explainer tool
        """
    ...
    def _generate_clarification_questions(self, extracted: ExtractedParameters) -> list[str]:
        ...
        for param in extracted.missing_parameters:
            if param == "topic":
                ...
                questions.append("What difficulty level would you prefer? (easy, medium, or hard)")
            elif param == "count":
